if (Meteor.isServer) {
  ...
  Meteor.publish("clientssummary", function () {
    ...
    if (ClientsSummary.find().count() === 0) {
      ...
      ClientsSummary.insert({_id: 'summaryId1234', activeLoggedUsers :0, totalLoggedUsers: 0, recordClients: 0, signedupUsers: 0, totalSpeakersLogged: 0, totalRegistered: 0, activeLastHour : 0, totalArrived: 0, totalInvited: 0, totalUninvited: 0, totalUninvitedModerators: 0});
    }
      Meteor.setInterval(function(){
        ...
        Clients.update({life: {$gt: 0}}, {$inc: {life: -10}}, {multi:true});
         ...
         var clientsSummary = ClientsSummary.findOne({}) || 0;
          ...
          var totalUninvitedModerators = Meteor.users.find({'roles.moderator': true, welcome: false}, {fields: {_id:1}}).count();
          ...
          var lTime = moment().subtract(60, 'm').toDate();
          var activeLastHour = Clients.find({lastseen : {$gt : lTime}}).count()

          var totalUninvited = Meteor.users.find({attendee: true, emailSent: false}, {fields: {_id: 1}}).count()
        ...
        ClientsSummary.update({_id : 'summaryId1234'}, {$set : {
          ...
          totalUninvitedModerators: totalUninvitedModerators
        }});
